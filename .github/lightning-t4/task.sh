...
python3 - "${NVIDIA_ICD}" <<'PY' | tee -a "${REPORT}"
import json, sys
with open(sys.argv[1], encoding="utf-8") as fh:
    data=json.load(fh)
library=data.get("ICD",{}).get("library_path")
if not isinstance(library,str) or not library:
    raise SystemExit("invalid NVIDIA ICD library path")
print(f"NVIDIA VULKAN ICD: {sys.argv[1]}")
print(f"NVIDIA VULKAN LIBRARY: {library}")
PY
