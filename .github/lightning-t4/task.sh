...
report "LAUNCH COMMAND: DISPLAY=${DISPLAY} VK_ICD_FILENAMES=${NVIDIA_ICD} LD_LIBRARY_PATH=${BIN}:<existing> ${command[*]}"
: >"${STDIO}"
setsid env DISPLAY="${DISPLAY}" VK_ICD_FILENAMES="${NVIDIA_ICD}" LD_LIBRARY_PATH="${BIN}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" "${command[@]}" >"${STDIO}" 2>&1 &
launcher_pid=$!
